...
6. 三维中直接点击生成的钻孔可检查其网络、尺寸和位置；**Refine in 3D** 固定该回路当前分段，可从可见钻孔整段悬停、选中和拖动，中点环仅为辅助。拖动结束后尽可能延长相连正交支路以保留交点，并运行精确草稿检查。端口位置不会被擅自移动；破坏入口封闭、壁厚、连接或通油开口的调整仍报 FAIL。**Hydraulic Nets** 区分连接意图和派生钻孔；优化比较正交轴顺序、入口和偏移路径，最多六个精确候选。Save & Validate 对自动路由（含已存候选）进行最多六个候选的精确比较，先比较实际 FAIL、WARNING，再比较加工代价。硬失败触发冲突定向搜索，可改动冲突任一网络或成对改动；冻结和手动钻孔保持不变。交互候选也将已知硬几何失败排在成本之前，详情见 [自动布路可行性修正](docs/ROUTING_FEASIBILITY.md)。代理风险仅用于安排精确候选，不能优先于已验证方案的加工成本。参数拖动预览仍明确标为未验证；默认偏好最小壁厚之外额外 4 mm 的余量，以软惩罚与钻深、钻孔数和堵头数权衡，可在 Block & Constraints 修改。最小壁厚硬规则不变。失败候选也保存到 `output/optimizations/`。优化期间草稿变化会阻止覆盖。该有限搜索不保证任意布局都找到可行路线。
7. **Save & Validate** 保存精确 OCCT 实体、验证结果和不可变构建。**Hydraulic nets** 显示按网络分别布尔并集后的液压体，默认保留透明阀块背景，Stock opacity 可调至 0；**Machined void** 显示从阀块实际去除的完整加工空腔；**Solid** 显示实体阀块，默认遵守深度遮挡，液压 X-ray 必须主动开启；**Feature layers** 分别检查孔腔加工、端口加工、液压窗口和闭合区。跨网络接触显示警示及可隔离的精确碰撞体。编辑立即切换为明确标识的近似预览，精确结果就绪后更新模型和路由树。预览使用最新快照、合并连续编辑，并且只显示未验证/未优化的当前候选，不运行完整路由搜索；构建和显式优化才执行对应搜索并保留证据。失败显示具体原因并保留上一份可用视图，详见 [预览修正记录](docs/PREVIEW_FOLLOWUP.md)。当前 PASS 才开放 STEP；源定义加工清单逐段保留原生信息，未绑定的堵头入口加工仍为 unresolved，`manufacturing_ready` 独立于几何结果。验收与复现命令见 [V1 correction acceptance](docs/V1_CORRECTION_ACCEPTANCE.md)。
