...
7. **Save & Validate** 保存精确 OCCT 实体、验证结果和不可变构建。**Hydraulic nets** 显示按网络分别布尔并集后的液压体；**Machined void** 显示从阀块实际去除的完整加工空腔；**Solid** 显示实体阀块，默认遵守深度遮挡，液压 X-ray 必须主动开启；**Feature layers** 分别检查孔腔加工、端口加工、液压窗口和闭合区。跨网络接触显示警示及可隔离的精确碰撞体。编辑立即切换为明确标识的近似预览，精确结果就绪后更新模型和路由树。预览不写持久候选证据；构建和显式优化才保留证据。当前 PASS 才开放 STEP；源定义加工清单逐段保留原生信息，未绑定的堵头入口加工仍为 unresolved，`manufacturing_ready` 独立于几何结果。验收与复现命令见 [V1 correction acceptance](docs/V1_CORRECTION_ACCEPTANCE.md)。
