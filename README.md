...
PASS 仅代表当前规则范围通过，**不是制造放行或承压认证**。材料为工程元数据；压力、疲劳、流量分配、压损、热、污染、完整刀具/夹具干涉、螺纹强度及密封性能尚未计算；新版提供有明确假设的最小孔径流速筛查。外部油口可引用已有真实加工定义，使用其阶梯、锥面、环槽和偏移切削；也可使用尺寸可配置的 Custom straight bore。未提供的标准尺寸不推定，螺纹牙型、接头安装密封和承压能力不因选择一个规格而得到认证。外部工具干涉使用声明的圆柱包络及有来源的安装边界；未提供的阀体高度、夹具和工具外形不推定。
