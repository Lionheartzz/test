...
export function libraryUI(ctx,{open,editor,insert}){
  ...
  return async function library(){
    ...
    const filters=element('div',null,'editor-grid');content.append(filters);const query={q:'',unit:get().project_context||'metric',kind:'cavity',family:'',manufacturer:'',cavity_type:'',thread:'',offset:0,limit:30};
    ...
    const load=guard(async()=>{
      ...
      if(mode==='converted'){
        ...
        for(const row of result.items){const card=element('section',null,'library-card');list.append(card);card.append(element('h3',row.name),element('p',`${row.manufacturer} · ${row.unit.toUpperCase()} · ${row.cavity_type}\n${row.thread}\n${row.id}`));
          ...
          if(row.kind==='cavity'&&!['P','PORT'].includes(row.cavity_type.toUpperCase())){
        ...
        list.replaceChildren();for(const item of entries.filter(x=>(showDeleted||!x.deleted)&&x.preferred&&JSON.stringify(x.definition).toLowerCase().includes(query.q.toLowerCase()))){
          ...
          card.append(element('p','Engineering role: '+definitionRole(def)));if(!item.deleted&&isCavity(def))action(card,'Insert cavity',()=>insert(def));action(card,'Edit revision',()=>editor(def));
    ...
    action(tabs,'Find compatible cavities',guard(async()=>{const entries=await api('/api/library');open('Cartridge → compatible cavities');const results=element('div');const search=field(content,'Cartridge model','',()=>{});const render=()=>{results.replaceChildren();let count=0;for(const e of entries.filter(x=>x.preferred&&!x.deleted&&isCavity(x.definition))){const d=e.definition;for(const c of d.compatible_cartridges||[])if(c.status!=='unconfirmed'&&c.model.toLowerCase().includes(search.value.toLowerCase())){count++;const card=element('section',null,'library-card');card.append(element('h3',c.model+' → '+d.label),element('p',c.status+' · '+c.source));action(card,'Insert compatible cavity',()=>insert(d));results.append(card);}}if(!count)results.append(element('p','No documented match in the local catalog. Select a cavity manually and keep compatibility under engineering review.'));};search.oninput=render;content.append(results);render();}));
    ...
    field(filters,'Record type',query.kind,v=>{const [kind,family]=v.split(':');query.family=family||'';update('kind',kind);},{cavity:'Cartridge cavities',port_definition:'External-port machining',footprint:'Footprints',o_ring_groove:'O-ring grooves',undercut:'Undercuts','tool:drill':'Drill tools','tool:flat_bottom_drill':'Flat-bottom drill tools','tool:spot_face':'Spot-face tools',material_stock:'Material stock',assembly_envelope:'Assembly envelopes'});
