...
export function workflows(ctx){
  ...
  function nets(){
    ...
    const advanced=element('details');advanced.append(element('summary','Advanced'));content.append(advanced);action(advanced,'Reset all routes to automatic',guard(async()=>{if(!confirm('This will remove manual/frozen routing geometry and return all hydraulic nets to automatic routing. Continue?'))return;const d=await post('/api/adopt-routing',get());change(()=>set(hydrateDesign(d,definitions())));nets();}));
