...
let draftCheckedSignature=null,projectEpoch=0,savePending=null,statusPollPending=false;
let viewMode='review',lastUsablePreview=null;
let state,draft,report,reportDesign,model,resolved,selection='block',dirty=false,busy=false,externalChange=false,viewer,history=[],future=[],dragStart,exactPreview=null;
let materialCatalog=[];
let modifierCatalog=[];
...
function change(fn){if(busy)return false;const before=cloneDesign(draft);try{fn();syncNets(draft);history.push(before);if(history.length>40)history.shift();future=[];markDirty();renderTree();select(selection);return true;}catch(e){draft=before;notice(e.message,true);select(selection);return false;}}
...
function showSolid(result,snapshot=draft,context=null){solidStatus();resolved=hydrateDesign({...snapshot,features:result.features},draft.library,draft.threads);viewer?.setReferences(result.features);viewer?.load(result.model,resolved);viewer?.setDesign(draft);exactPreview=context&&result.model.deferred_layers?.length?{...context,designRevision:result.design_revision,pending:new Set()}:null;lastUsablePreview={model:result.model,design:resolved};renderTree();select(selection);previewState('exact-proposal','EXACT BREP · CURRENT PROPOSAL · NOT VALIDATED / NOT OPTIMIZED'+(result.model.brep_valid===false?' · BREP TOPOLOGY INVALID':''));notice(result.model.brep_valid===false?'Exact preview ready. Current proposal has invalid BRep topology; Validate reports engineering failures.':'Exact preview ready. Current proposal is not validated or optimized.');requestLayer(viewMode);}
const previews=createPreviewQueue({post,stream:streamExactPreview,
  ...
  onFast(p,snapshot){exactPreview=null;resolved=hydrateDesign(p.design,draft.library,draft.threads);viewer?.preview(resolved);viewer?.setReferences(resolved.features);viewer?.setDesign(draft);lastUsablePreview={design:resolved};renderTree();previewState('approximate','APPROXIMATE LIVE VIEW · CURRENT PROPOSAL · NOT VALIDATED');},
  onExact:showSolid,
  ...
  onError(error){exactPreview=null;if(lastUsablePreview){const p=lastUsablePreview;if(p.model)viewer?.load(p.model,p.design);else viewer?.preview(p.design);viewer?.setDesign(p.design);}solidStatus();previewState('unavailable','EXACT PREVIEW UNAVAILABLE · PREVIEW FAILED · LAST USABLE VIEW RETAINED · '+(lastUsablePreview?.model?'EXACT':'APPROXIMATE')+' · '+error.message);notice('Preview: '+error.message,true);}
});
async function requestLayer(mode){
  const layer=mode==='void'?'void':mode==='features'?'features':null,token=exactPreview;
  if(!layer||!token||!viewer?.needsLayer(layer)||token.pending.has(layer))return;
  token.pending.add(layer);const started=performance.now();
  try{const result=await post('/api/preview-layer',{design_revision:token.designRevision,layer},{headers:{'X-PMC-Preview-Owner':token.owner,'X-PMC-Preview-Version':String(token.version)}});if(exactPreview!==token||result.design_revision!==token.designRevision||result.layer!==layer)return;viewer.loadLayer(layer,result.parts);previewTiming({kind:'lazy-layer-request',layer,total_ms:performance.now()-started});}
  catch(error){if(exactPreview===token)notice('Exact inspection layer unavailable: '+error.message,true);}
  finally{token.pending.delete(layer);}
}
function requestSolid(){
  if(!draft||dragStart||busy)return;
  if(!dirty&&!state?.stale&&state?.build){exactPreview=null;previews.cancel();if(model){viewer?.load(model,resolved);lastUsablePreview={model,design:resolved};}else{viewer?.preview(resolved);lastUsablePreview={design:resolved};}viewer?.setDesign(draft);previewState('authoritative','AUTHORITATIVE VALIDATION · '+state.build.status+' · '+(model?'EXACT MACHINED BREP':'EXACT REVIEW UNAVAILABLE · APPROXIMATE VIEW'));return;}
  previews.schedule(draft,String(projectEpoch));
}
function refreshPreview(){
  solidStatus();
  try{viewer?.preview(draft);viewer?.setDesign(draft);previewState('approximate','APPROXIMATE LIVE VIEW · NOT VALIDATED');}
  catch(error){$('model-info').textContent='LAST USABLE VIEW RETAINED · '+error.message;}
  if(dragStart)previews.cancel();else requestSolid();
}
function markDirty(){draftCheckedSignature=null;dirty=true;exactPreview=null;renderHeader();refreshPreview();notice('Draft preview updated. Validate runs exact geometry checks and writes an immutable build.');}
try{viewer=createViewer($('viewport'),select,(id,u,v,done)=>{if(busy)return;const f=draft.features.find(f=>f.id===id),starting=!dragStart;if(starting){dragStart=cloneDesign(draft);previews.cancel();exactPreview=null;dirty=true;renderHeader();}f.u=u;f.v=v;$('selection-label').textContent=`${featureLabel(f,draft)} · ${f.face} · U ${u.toFixed(1)} / V ${v.toFixed(1)} mm`;notice('Live position · envelope clamped to face · 1 mm snap · preview is not validated');if(!done){viewer?.updateFeature(draft,id);previewState('approximate','APPROXIMATE LOCAL DRAG · NOT VALIDATED');return;}const baseline=dragStart;history.push(baseline);future=[];dragStart=null;select(id);if(f.frozen_net)refineAfterDrag(baseline,id,u,v);else refreshPreview();});$('viewport').addEventListener('pmc-viewer-timing',event=>previewTiming(event.detail));}catch(e){notice('WebGL: '+e.message,true);}
...
async function adoptRoute(netId,id=selection){
  ...
  try{
    ...
    if(displayed?.model)viewer?.load(displayed.model,resolved);else viewer?.preview(resolved);
...
function renderHeader(){
  ...
  for(const [id,name]of [['step-download','production.step'],['engineering-step-download','engineering.step'],['data-download','design.json'],['report-download','validation.md'],['chart-download','drill-chart.csv'],['manufacturing-download','manufacturing.json']]){
    ...
    if(ok)link.href=`/api/artifacts/${state.build.build_id}/${name}`;else link.removeAttribute('href');
  }
  $('build').textContent=busy?'Computing exact solids…':'Validate';
  $('undo').disabled=busy||!history.length;
  $('redo').disabled=busy||!future.length;
  if(report&&draftCheckedSignature===JSON.stringify(draft))$('check-counts').textContent=`Draft exact checks · ${report.counts.PASS} PASS / ${report.counts.WARNING} WARNING / ${report.counts.FAIL} FAIL · not saved`;
  else if(dirty||state.stale||externalChange)$('check-counts').textContent=report?`Previous build · ${report.counts.PASS} PASS / ${report.counts.WARNING} WARNING / ${report.counts.FAIL} FAIL · draft not validated`:'Not validated';
}
...
function renderTree(){
  ...
  const generated=resolved?.features.filter(f=>f.route_net&&!draft.features.some(x=>x.id===f.id))||[];
  if(generated.length){
    const group=element('div',null,'tree-group generated-group');
    group.append(element('div',`Generated Routes · ${generated.length} · Current proposal`,'tree-heading'));
    for(const f of generated){
      const btn=element('button',null,'tree-feature'),dot=element('span',null,'swatch'),net=resolved.nets.find(n=>n.id===f.route_net);
      ...
      btn.append(dot,element('span',featureLabel(f,resolved)),element('small',displayNetName(resolved,f.route_net)));
      btn.onclick=()=>select(f.id);group.append(btn);
    }
    tree.append(group);
  }
...
function organizeInspector(kind){
  ...
  let current='',section=null,body=null;
  const opening=(name)=>{
    ...
    summary.addEventListener('click',()=>inspectorOpenByKind.set(key,!section.open));
  ...
  for(const node of nodes){
    ...
    if(node.tagName==='DETAILS'&&node.querySelector('summary')?.textContent==='Advanced / Developer information'){
      ...
      node.querySelector('summary').addEventListener('click',()=>inspectorOpenByKind.set(key,!node.open));
      form.append(node);continue;
    }
    const label=node.textContent?.trim()||'';
    let name=current;
    if(kind==='block'){
      if(!name)name='Block dimensions';
      if(label.startsWith('Material /')||label.startsWith('Engineering material'))name='Material & stock';
      if(label.startsWith('Allowable material stress'))name='Engineering rules';
      if(label.startsWith('Engineering notes'))name='Machining & notes';
    }else if(kind==='generated')name='Route details';
    else{
      if(!name)name='Position';
      if(label.startsWith('Face')||node.classList?.contains('field-row'))name='Position';
      else if(name==='Position'&&node!==nodes[0])name=kind==='cavity'?'Definition & cartridge':kind==='mounting'?'Mounting specification':'Hydraulic & machining';
      if(kind==='cavity'&&label.includes('→ Net'))name='Hydraulic interfaces';
    }
    opening(name);
    body.append(node);
  }
...
function finishSelection(kind,previous){
  ...
  if(report){
    const heading=draftCheckedSignature===JSON.stringify(draft)?'Draft exact checks · not saved':dirty||state?.stale||externalChange?'Previous build results':'Current build results';
...
function renderReport(){
  ...
  for(const t of ['Status','Rule','Items','Actual','Required'])head.append(element('th',t));
  table.append(head);
  for(const c of checks){
    ...
    row.append(
      ...
      element('td',c.items.map(item=>displayIdentity(draftCheckedSignature===JSON.stringify(draft)?resolved||draft:reportDesign||resolved||draft,item)).join(' ↔ ')),
      element('td',`${c.actual} ${c.unit}`),
      element('td',String(c.required))
    );
    row.onclick=()=>{
      const target=c.items.map(item=>item.split(':')[0]).find(id=>
        draft.features.some(f=>f.id===id)||resolved?.features.some(f=>f.id===id));
      if(target)select(target);
      else notice('This result does not identify a feature in the current draft.',true);
    };
    table.append(row);
  }
  container.append(table);
}
async function load(key=state?.project_id){if(!key)return;const preserveView=key===state?.project_id&&!document.body.classList.contains('home');const epoch=++projectEpoch;previews.cancel();lastUsablePreview=null;exactPreview=null;const next=await api('/api/projects/'+key);let nr,nm,nd,reviewError;if(next.build)[nr,nm,nd]=await Promise.all(['validation.json','review.json','resolved_design.json'].map(n=>api(`/api/artifacts/${next.build.build_id}/${n}`).catch(()=>null)));if(epoch!==projectEpoch)return;if(nm?.geometry_kind==='unavailable'){reviewError=nm.review_error;nm=null;}state=next;const networkLabel=document.querySelector('.local-dot');networkLabel.textContent=next.network?.mode==='lan'?'LAN workspace':'Local workspace';networkLabel.title=(next.network?.urls||[]).join('\n');draft=hydrateDesign(structuredClone(next.design),next.engineering?.definitions,next.engineering?.threads);report=nr;reportDesign=nd?hydrateDesign(nd,next.engineering?.definitions,next.engineering?.threads):null;model=next.stale?null:nm;resolved=next.stale?draft:hydrateDesign(nd||draft,next.engineering?.definitions,next.engineering?.threads);viewer?.setReferences(resolved.features);dirty=false;externalChange=false;previews.cancel();if(!model){viewer?.preview(draft);lastUsablePreview={design:cloneDesign(draft)};}if(model){lastUsablePreview={model,design:resolved};viewer?.load(model,resolved);previewState('authoritative',`AUTHORITATIVE VALIDATION · ${next.build.status} · EXACT OCCT BREP · ${(model.volume_mm3/1000).toFixed(1)} cm³`);}viewer?.setDesign(draft);document.body.classList.remove('home');if(!preserveView)viewer?.fit($('face-view').value);if(!model)refreshPreview();if(!dragStart)requestSolid();renderTree();select(selection);renderReport();renderHeader();notice(reviewError&&!state.stale?reviewError+' Authoritative Validate: '+state.build.status:state.stale?'Project is newer than this build. Validate to update.':'Select a cavity, external port, or hydraulic route.');}
async function build(){
  ...
  try{
    ...
    shell.setBusy(false);renderHeader();
  }
}
