...
function renderHeader(){
  ...
  else if(dirty||state.stale||externalChange)$('check-counts').textContent=report?`${reportWasDraft?'Previous draft exact checks':'Previous build'} · ${report.counts.PASS} PASS / ${report.counts.WARNING} WARNING / ${report.counts.FAIL} FAIL · draft not validated`:'Not validated';
}
...
function finishSelection(kind,previous){
  ...
  if(report){
    const heading=reportSource({report,build:state?.build,dirty,stale:state?.stale,externalChange,draftCheckedSignature,draftSignature:JSON.stringify(draft),reportWasDraft}).label;
...
function renderReport(){
  ...
  const source=reportSource({report,build:state?.build,dirty,stale:state?.stale,externalChange,draftCheckedSignature,draftSignature:JSON.stringify(draft),reportWasDraft});
  ...
  viewer?.setIssueMarkers(source.spatial?issueReferences(report,{displayed:resolved||draft,draft}):[]);
  ...
  for(const c of checks){
    ...
    row.title=c.message;
    const targets=resolveCheckTargets(c,{displayed:resolved||draft,draft});
    row.append(
    ...
    if(targets.ids.length){
      const locate=action(sourceCell,source.spatial?'Locate affected objects':'View current object reference',()=>{
        ...
        else if(!source.spatial)notice('Showing current object references. These results belong to a previous build.');
      });
