...
export function createViewer(container, onSelect, onDrag = ()=>{}) {
  ...
  function addBoundaries(f){
    if(!f.definition||f.suppressed)return;const def=design.library.find(d=>d.id===f.definition),p=pose(f,design.block),[u,v]=axes[f.face],angle=(f.rotation||0)*Math.PI/180;
  ...
  function addControl(f){
    ...
    const def=design.library?.find(x=>x.id===f.definition),target=new THREE.Mesh(f.kind==='drilling'?new THREE.CylinderGeometry(1,1,f.depth,24):new THREE.CircleGeometry(1,32),new THREE.MeshBasicMaterial({side:THREE.DoubleSide,transparent:true,opacity:0,depthWrite:false,colorWrite:false}));
  ...
  function parameterModel(value,onlyIds=null,includeBlock=true) {
    ...
    for(const f of value.features.filter(f=>!f.suppressed&&(!onlyIds||onlyIds.has(f.id)))) {placements[f.id]=pose(f,b);if(f.definition){const d=value.library.find(d=>d.id===f.definition);for(const [i,s] of (d.cutting_primitives?.length?d.cutting_primitives:d.stages).entries())cylinder(f,s.diameter,s.start,s.end,f.kind==='port'?(value.nets.find(n=>n.id===f.circuit)?.color||'#9cc8e8'):'#b4c7da',f.kind==='port'?'port':'cavity',f.id+'-'+i,s);for(const z of f.kind==='port'?[]:d.zones){const net=value.nets.find(n=>n.id===f.circuits[z.id]);cylinder(f,z.diameter,z.start,z.end,net?.color||({P:'#ef5959',T:'#459cff',A:'#41ca8b',B:'#f2d454'}[f.circuits[z.id]])||'#b08bea','zone',f.id+':'+z.id,z);}}else{cylinder(f,f.diameter,0,f.depth,value.nets.find(n=>n.id===f.circuit)?.color||({P:'#ef5959',T:'#459cff',A:'#41ca8b',B:'#f2d454'}[f.circuit])||'#b08bea',f.kind,f.id);const tip=f.tip_angle===180?0:f.diameter/2/Math.tan((f.tip_angle??118)*Math.PI/360);if(tip)cylinder(f,f.diameter,f.depth,f.depth+tip,value.nets.find(n=>n.id===f.circuit)?.color||'#b08bea',f.kind,f.id+':tip',{kind:'cone',end_diameter:0});if(f.plugged)cylinder(f,f.diameter,0,f.plug_length,'#d5dee9','plug',f.id+':plug');}}
  ...
  function updateFeature(value,id){
    ...
    const current={...value,features};design=current;dragOwners=affected;disposeGroup(dragGroup);
