...
export function portSetup(ctx,parent,port,label,redraw,context=ctx.get?.().project_context||'metric'){
  ...
  field(card,label+' · Machining',port.mode,v=>{port.mode=v;port.definition=null;redraw();},{custom:'Custom straight bore',catalog:'SQLite external-port definitions'});
  if(port.mode==='custom'){
    ...
    if(ctx.createCustomExternalPort)action(card,'Create reusable custom external port',()=>ctx.createCustomExternalPort({onSelect:definition=>{port.mode='catalog';port.definition=definition;port.size=definition.thread_note||definition.label;redraw();},actionLabel:'Use External Port'}));
    return;
  }
  ...
  if(ctx.externalPortLibrary)action(card,'Browse External Port Library',()=>ctx.externalPortLibrary({onSelect:definition=>{port.definition=definition;port.size=definition.thread_note||definition.label;redraw();},actionLabel:'Use External Port'}));
  ...
  async function search(){const token=++request;results.replaceChildren(element('p','Loading port definitions…','loading-state'));results.setAttribute('aria-busy','true');try{
    const rows=(await api('/api/catalog?'+new URLSearchParams({kind:'port_definition',unit:port.unit,q:port.search,limit:30}))).items;
