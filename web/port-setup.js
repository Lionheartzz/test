...
export function portSetup(ctx,parent,port,label,redraw,context=ctx.get?.().project_context||'metric'){
  ...
  if(port.definition){card.append(element('p',port.definition.label+' · '+port.definition.id),element('p','Exact runtime machining profile from the engineering database.'));action(card,'Change '+label+' definition',()=>{port.definition=null;redraw();});return;}
