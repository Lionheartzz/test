...
export function aiGeneration(ctx, {open, back, session, refreshTask, watchJob}) {
  ...
  function renderPlan(task,run,plan,attempted=false){
    ...
    if(plan.mounting_requirements?.length>1){
      ...
      const choices=Object.fromEntries([['','Unassigned'],...plan.mounting_requirements.map(row=>[row.intent_id,row.intent_id+' · '+(row.mounting?.thread_designation||'thread unresolved')])]);
