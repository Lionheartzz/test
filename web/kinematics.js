...
export function smartAlign(f,design,u,v,tolerance=2){
  ...
  const guides=[];const values=[u,v].map((value,i)=>{const axis=[au,av][i],sorted=refs.map(r=>({...r,gap:Math.abs(r.p[axis]-value)})).sort((a,b)=>a.gap-b.gap);if(sorted[0]?.gap<=tolerance){guides.push({axis,value:sorted[0].p[axis],label:sorted[0].label});return sorted[0].p[axis];}return Math.round(value);});return {values:clamp(f,design,...values,0),guides};
}
