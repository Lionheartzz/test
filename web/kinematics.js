...
export function bounds(f,design) {
  const definitionId=f.cavity_id||f.port_definition_id;
  const d=design.library.find(d=>d.id===definitionId);
...
export function clamp(f,design,u,v,snap=1) { const b=bounds(f,design);if(!b.fits)throw Error(`${f.id||f.kind}: Component envelope does not fit on ${f.face} face`);const round=n=>snap?Math.round(n/snap)*snap:n;return [Math.max(b.minU,Math.min(b.maxU,round(u))),Math.max(b.minV,Math.min(b.maxV,round(v)))]; }
export function syncNets(d) {
  const members=Object.create(null);for(const f of d.features) {if(f.kind==='cavity')for(const [z,n]of Object.entries(f.interface_nets||{}))(members[n]??=[]).push(`${f.id}:${z}`);else if(f.kind==='port')(members[f.circuit]??=[]).push(f.id);}
