...
def test_corrected_demo_passes_and_step_roundtrips(tmp_path):
    ...
    assert set(report['graph']['G-P']) == {'P', 'CV1:upper', 'RV1:upper', 'XD-P'}
    assert 'CV1:lower' not in report['graph']['CV1:upper']
...
def test_true_six_mm_wall_is_detected():
    ...
    assert hit[0]['actual'] == pytest.approx(6)
...
def test_installation_envelope_interference():
    d = demo().model_dump(); d['library'][0]['clearance_diameter'] = 50
...
@pytest.mark.parametrize('mutation', ['duplicate', 'unknown_target', 'nan', 'negative', 'unknown_field', 'zones', 'stage_gap'])
def test_schema_rejects_invalid_engineering_data(mutation):
    ...
    if mutation == 'stage_gap': d['library'][0]['stages'][1]['start'] = 11
