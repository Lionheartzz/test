...
Save & Validate evaluates the current candidate exactly and retains its resolved design, constructed solids, authorized contacts and complete validation report. If feasible, it goes directly to STEP export/reimport and review generation. If infeasible, the existing bounded conflict-directed search remains available. The selected candidate's already computed solids/report are reused within that calculation only. Explicit Optimize Routes still performs its requested bounded search. There is no cross-draft validation cache or approximation replacing BRep authority.
