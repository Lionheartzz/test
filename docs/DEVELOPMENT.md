...
- The viewer ignores zero-size resize notifications and defers camera fit until measurable. Editing replaces previous exact geometry with an identified parametric preview until a matching BRep arrives. Solid uses true depth; hydraulic overlays require explicit X-ray. Refined segments retain full-length picking and centerline references to automatic routes during gestures.
...
Definition roles explicitly separate cartridge cavities and external-port machining. Legacy roles derive from immutable source classification; changing role requires a written engineering decision. Shared and pinned selection workflows filter the role; UUID port IDs keep user net labels independent. External-port search defaults to project native units. Default automatic routes compare bounded exact candidates before cost selection. Selection is transient unless a build explicitly opts into evidence under `output/route-selections/`; explicit Optimize Routes owns one total exact-attempt budget including its baseline. The real-browser continuous-drag regression runs with `node scripts/check-viewer-lifecycle.mjs`.
