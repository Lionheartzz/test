...
def database_path() -> Path:
    ...
    return Path(value).expanduser().resolve() if value else DEFAULT_DB
...
def definitions(ids: set[str] | list[str]) -> dict[str, CavityDefinition]:
    result = {}
    for identifier in sorted(set(ids)):
        result[identifier] = get_definition(identifier)
    return result


def definitions_for_design(design) -> dict[str, CavityDefinition]:
    ids = {
        identifier
...
def compatible_cavity_ids(cartridge_id: str):
    with _connect() as connection:
        return [row[0] for row in connection.execute(
            "SELECT cavity_id FROM cartridge_cavities WHERE cartridge_id=? AND valid=1 ORDER BY cavity_id",
            (cartridge_id,))]
...
def validate_references(design) -> None:
    resolved = definitions_for_design(design)
    with _connect() as connection:
        ...
        for cartridge_id in cartridge_ids:
            if connection.execute(
                "SELECT 1 FROM cartridges WHERE id=? AND active=1", (cartridge_id,)
            ).fetchone() is None:
                raise ValueError(f"Cartridge does not exist or is inactive: {cartridge_id}")
        for feature in design.features:
